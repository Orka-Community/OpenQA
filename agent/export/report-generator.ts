...
export class ReportGenerator {
  ...
  async generateExecutiveReport(sessionId: string): Promise<string> {
    // Génère un rapport HTML 1 page, design propre, imprimable, sans JS
    // Sections:
    //   1. En-tête: App name, URL, date, durée session
    //   2. Score global (grand cercle coloré 0-100)
    //   3. 3 métriques clés: bugs critiques, pages testées, spécialistes actifs
    //   4. Top 5 issues critiques (titre + sévérité + extrait description)
    //   5. Recommandations par priorité (max 5 bullets)
    //   6. Signature: "Generated by OpenQA — Autonomous QA Agent"
    //
    // Sauvegarde dans ./data/reports/executive_{sessionId}_{timestamp}.html
    // Retourne le chemin du fichier

    const report = await this.generateReport(sessionId);
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `executive_${sessionId.slice(-8)}_${timestamp}.html`;
    const filepath = join(this.reportsDir, filename);

    // Couleur selon score
    const scoreColor = report.overallScore >= 75 ? '#22c55e'
                     : report.overallScore >= 50 ? '#f59e0b'
                     : '#ef4444';

    // Top 5 critical issues
    const topIssues = report.bugs
      .filter((b: any) => b.severity === 'critical' || b.severity === 'high')
      .slice(0, 5);

    // Toutes les recommendations des sections
    const allRecs = Object.values(report.sections)
      .flatMap((s: any) => s.recommendations || [])
      .filter(Boolean)
      .slice(0, 5);
...
<title>QA Executive Report — ${report.applicationName}</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f8fafc; color: #1e293b; padding: 40px; max-width: 900px; margin: 0 auto; }
  @media print { body { background: white; padding: 20px; } .no-print { display: none; } }
  h1 { font-size: 24px; font-weight: 700; }
  h2 { font-size: 16px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: #64748b; margin-bottom: 12px; }
  ...
  .score-circle { width: 80px; height: 80px; border-radius: 50%; background: ${scoreColor}; color: white; display: flex; align-items: center; justify-content: center; font-size: 26px; font-weight: 800; margin: 0 auto 8px; }
  .score-label { font-size: 12px; color: #64748b; }
  .metrics { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin: 28px 0; }
  .metric-card { background: white; border-radius: 12px; padding: 20px; box-shadow: 0 1px 3px rgba(0,0,0,.08); text-align: center; }
  ...
  .issue-row { display: flex; align-items: flex-start; gap: 12px; padding: 12px 0; border-bottom: 1px solid #f1f5f9; }
  ...
  .rec-list li { padding: 8px 0; border-bottom: 1px solid #f1f5f9; font-size: 14px; display: flex; gap: 10px; }
  .rec-list li:last-child { border-bottom: none; }
  .rec-list li::before { content: "→"; color: #6366f1; font-weight: 700; flex-shrink: 0; }
...
<div class="header">
  ...
  <div class="score-block">
    ...
    <div class="score-label">QA SCORE / 100</div>
  </div>
</div>

<div class="metrics">
  <div class="metric-card">
    <div class="metric-value" style="color:${report.metrics.criticalBugs > 0 ? '#dc2626' : '#22c55e'}">${report.metrics.criticalBugs}</div>
    <div class="metric-label">Critical Issues</div>
  </div>
  <div class="metric-card">
    <div class="metric-value">${report.metrics.pagesVisited}</div>
    <div class="metric-label">Pages Tested</div>
  </div>
  <div class="metric-card">
    <div class="metric-value">${report.metrics.specialistsRun}</div>
    <div class="metric-label">Specialists Run</div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="issue-row">
    ...
    <div>
      ...
      <div class="issue-desc">${(b.description || '').slice(0, 120)}${(b.description || '').length > 120 ? '…' : ''}</div>
    </div>
  </div>`).join('')}
</div>` : `
<div class="section">
  <h2>Issues Found</h2>
  <p style="color:#22c55e;font-weight:600">&#10003; No critical or high-severity issues detected in this session.</p>
</div>`}
...
<div class="section">
  ...
  <ul class="rec-list">
    ${allRecs.map((r: string) => `<li>${r}</li>`).join('')}
  </ul>
</div>` : ''}

<div class="footer">
  Generated by <strong>OpenQA</strong> — Autonomous QA Agent &nbsp;·&nbsp; ${new Date().toISOString()}
</div>
